  ...
  return (
    <div className="max-w-2xl mx-auto bg-white rounded-lg shadow-lg p-6">
      ...
      <div className="text-center mb-6">
        ...
        <p className="text-3xl font-bold text-blue-800 mb-6">{card.term}</p>
            ...
            <div key={card.id} className={`p-4 rounded-lg border ${
              ...
              <div className="text-sm space-y-1">
                <div>
                  ...
                  <span className="font-medium text-green-600">{card.definition}</span>
                </div>
                {!card.isCorrect && (
                  <div>
...
const Quiz: React.FC = () => {
  ...
  // Tạo các lựa chọn sai ngẫu nhiên
  const generateWrongOptions = (correctAnswer: string, allDefinitions: string[]): string[] => {
    const wrongOptions = allDefinitions
      .filter(def => def !== correctAnswer)
      .sort(() => Math.random() - 0.5)
      .slice(0, 3);
    
    // Nếu không đủ 3 lựa chọn sai, thêm một số lựa chọn mặc định
    const defaultWrongOptions = [
      "Máy tính", "Bàn ghế", "Cửa sổ", "Điện thoại", "Sách vở",
      "Xe đạp", "Tivi", "Quần áo", "Giày dép", "Túi xách"
    ];
  ...
  useEffect(() => {
    const fetchVocab = async () => {
      if (!vocabId) {
        ...
        const allDefinitions = demoData.map(item => item.definition);
        
        const quizCards: QuizCard[] = demoData.map((vocab, index) => {
          const wrongOptions = generateWrongOptions(vocab.definition, allDefinitions);
          const allOptions = [vocab.definition, ...wrongOptions].sort(() => Math.random() - 0.5);
      ...
      try {
        ...
        const allDefinitions = vocabList.map(vocab => vocab.definition);
        
        const quizCards: QuizCard[] = vocabList.map((vocab, index) => {
          const wrongOptions = generateWrongOptions(vocab.definition, allDefinitions);
          const allOptions = [vocab.definition, ...wrongOptions].sort(() => Math.random() - 0.5);
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4">
        <h1 className="text-4xl font-bold text-center mb-8">Quiz Từ Vựng</h1>
        ...
        {isCompleted ? (
          ...
          <div className="max-w-2xl mx-auto bg-white rounded-lg shadow-lg p-8">
            ...
            <p className="text-sm text-gray-500 text-center mt-6">
              Chọn đáp án đúng cho từ vựng trên
            </p>
          </div>
        )}
      </div>
